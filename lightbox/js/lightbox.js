(function(){
    ...
    // }
    //初始化
    window.lightbox.init=function (obj){
        let selector=obj.selector;
        let rewrite=obj.rewrite;
        let srcArr=[],
            textArr=[];
        createLightBox(selector);
        if(obj.srcArr!==undefined&&obj.textArr!==undefined){
            srcArr=obj.srcArr;
            textArr=obj.textArr;
            createFigureList(selector,srcArr,textArr,rewrite);
            lightBox(selector);
        }else{
            lightBox(selector);
        }
    ...
    //创建内容列表
    function createFigureList(selector,srcArr,textArr,rewrite){
